...
suite("coerceEnum", () => {
  test("returns the value when it's a member of the allowed set", () => {
    assert.strictEqual(coerceEnum("server", MODES, "direct", "mode"), "server");
  ...
  test("falls back on undefined", () => {
    assert.strictEqual(coerceEnum(undefined, MODES, "direct", "mode"), "direct");
  });

  test("falls back on null", () => {
    assert.strictEqual(coerceEnum(null, MODES, "direct", "mode"), "direct");
  });

  test("falls back on non-string types", () => {
    assert.strictEqual(coerceEnum(42, MODES, "direct", "mode"), "direct");
    assert.strictEqual(coerceEnum(true, MODES, "direct", "mode"), "direct");
    assert.strictEqual(coerceEnum({ x: 1 }, MODES, "direct", "mode"), "direct");
  });
